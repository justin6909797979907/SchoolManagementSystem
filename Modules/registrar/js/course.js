  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    */
    ...
    document.getElementById('addCourseSubmit').addEventListener('click', function() {
    ...
    */
    ...
    fetch(`${BASE_URL}/course/all?order=${order}&limit=${limit}&page=${page}&search=${encodeURIComponent(search)}`)
        .then(response => response.json())
        .then(result => {
            ...
            result.data.forEach(course => {
                tbody.innerHTML += `
                    <tr class="activity-row">
                     ...
                     <td><input type="checkbox" class="activity-checkbox" value="${course.id}"></td>
                        <td>${course.code}</td>
                        <td>${course.name} </td>
                        <td>${course.years}</td>
